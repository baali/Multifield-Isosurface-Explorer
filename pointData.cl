...
__kernel void pointSet(__global Point* point, __global int *batchCount,
		       __global int *kappaFlag, __global float range[],
		       __global float *inc, __global int *XDIM,
		       __global int *YDIM, __global int *ZDIM,
		       __global float (*bins)[110], 
		       __global float (*binst)[110])
{
  ...
  unsigned int ids[8];
  ids[0] = (id % (*XDIM - 1)) +
    ((id / (*XDIM - 1))%(*YDIM - 1)) * (*XDIM) +
    (id / ((*XDIM - 1) * (*YDIM - 1))) * (*XDIM) * (*YDIM);
  ids[1] = ids[0] + 1;
  ...
  // float4 v[4];
  // float fs[4], gs[4], hs[4];
  float4 v[24];
  float fs[24], gs[24], hs[24];
  int indices[] = {0, 1, 5, 6, 0, 1, 2, 6, 0, 2, 3, 6, 
		   0, 4, 5, 6, 0, 4, 7, 6, 0, 3, 7, 6};
  for (int j = 0; j < 24; j++)
    {
      v[j] = point[ids[indices[j]]].v;
      fs[j] = point[ids[indices[j]]].fs;
      gs[j] = point[ids[indices[j]]].gs;
      hs[j] = point[ids[indices[j]]].hs;
    }
  for(int j = 0; j < 110; j++)
    {
      bins[id][j] = 0;
  ...
  for (int p = 0; p < 6; p++)
    {
      // for (int j = 0; j < 4; j++)
      //   {
      //     v[j] = point[ids[indices[p*4 + j]]].v;
      //     fs[j] = point[ids[indices[p*4 + j]]].fs;
      //     gs[j] = point[ids[indices[p*4 + j]]].gs;
      //     hs[j] = point[ids[indices[p*4 + j]]].hs;
      //   }
      float kappa = 1;
      if (*kappaFlag == 1)
  	{
  	  // float4 gradF = FindGradient (&v[0], &fs[0]);
  	  // float4 gradG = FindGradient (&v[0], &gs[0]);
      ...
      PlotKappaForTet (&v[p*4], &fs[p*4], &gs[p*4], &hs[p*4], kappa, *inc, range, bins[id], binst[id]);
      // PlotKappaForTet (&v[0], &fs[0], &gs[0], &hs[0], kappa, *inc, range, bins[id], binst[id]);
    }
}
...
__kernel void summ(__global float (*binR)[110], 
		   __global float (*binstR)[110], 
		   __global float bins[110], 
		   __global float binst[110], 
		   __global int *p)
{
  //get our index in the array
  /* unsigned int id = (*batchCount * 10000000) + get_global_id(0); */
  unsigned int id = get_global_id(0);
  /* unsigned int i = get_global_id(0);     */
  // bins[i] = binst[i] = 0;
  for(int j = 0; j < *p; j++)
    {
      bins[id] += binR[j][id]; 
      binst[id] += binstR[j][id]; 
    }
  /* if (id == (*batchCount * 10000000)) */
  /*   { */
  /*     batchCount += 1; */
  /*   } */
}
